 ...
 * @since  1.6
 */
class JoompushModelSubscribers extends JModelList
{
/**
	* Constructor.
	*
	...
	*/
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__joompush_subscribers` AS a');
		...
		// Join over the user field 'user_is'
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.user_id LIKE ' . $search . '  OR  a.browser LIKE ' . $search . ' )');
			}
		}
		
		//to get group subscriber
		$jinput = JFactory::getApplication()->input;
		$gid = $jinput->get('gid', 0, 'INT');
		
		if ($gid) 
		{
			$query->select('`gm`.id AS `gm`');
			$query->join('LEFT', '#__joompush_subscriber_group_map AS `gm` ON `gm`.subscriber_id = a.`id` AND `gm`.group_id = ' . $gid);
		}
