...
class ContentModelAixeenaTicker extends JModelList
{
	/**
	 * Constructor.
	 *
...
'
		...
		if ($this->tags)
		{
			$query->join('INNER', '#__contentitem_tag_map AS ctm ON a.id = ctm.content_item_id');
			$query->join('INNER', '#__tags AS t ON t.id = ctm.tag_id');
		...
		if($date_range) $query->where('( a.publish_up BETWEEN SYSDATE() - INTERVAL '.$date_range.' DAY AND SYSDATE())');
