...
if ($this->value && in_array($params->get('params.view_mail', 1), $this->user->getAuthorisedViewLevels()))
{
	...
	echo '<div class="emailadres"><ul class="list">';

	$list_item = '<li class="list__item"><svg class="list__icon icon icon--email" aria-hidden="true" focusable="false"><use href="#ui__envelope-alt"></svg><span list__title">%s</span></li>';
	...
	if (is_array($this->value))
	{
		...
		{
			echo sprintf($list_item, JHtml::_('content.prepare', $email));
		}
	}
	// * если email 1
	else
	{
		echo sprintf($list_item, JHtml::_('content.prepare', $this->value));
	}
