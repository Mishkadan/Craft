...
class JFormFieldCGallery extends CFormFieldUpload
{
	public function getInput()
	{
		...
		// * фнкция добавления сортировки по порядку к массиву файлов
		$this->_addOrder();
	...
	public function _addOrder()
	{
		if (count($this->value) > 1) {
			...
			$_id_array = [];
			$result = []; // * финальный массив
			$order = $this->value; // * список файлов

			// * получаем id всех картинок
			foreach ($order as $key => $value) {
				$_id_array[$key] = $value['id'];
			}
			// * подключение к базе
			$db = JFactory::getDbo();
			...
			// * формируем запрос
			$query
				->select($db->quoteName(array('id', 'order')))
				->from($db->quoteName('#__js_res_files'))
				->where($db->quoteName('id') . ' IN (' . implode(',', array_map('intval', $_id_array)) . ')');

			// * отправляем запрос
			$db->setQuery($query);
			$order = $db->loadObjectList('id');

			// * модифицируем массив файлов, добавляя order
			foreach ($this->value as $key => $file) {
				$this->value[$key]['order'] = $order[$this->value[$key]['id']]->order ?? 0;
			}
		}
	...
	private function _drawRow($pictures, $height, $client)
	{
		...
		$patern_img   = '<div class="image-wrapper galrol" style="width:%spx;background-size: cover;background-image:url(\'%s\');"><a id="k%s" title="%s" href="%s" ' . $rel . '><img class="image-element" src="%s" border="0" /></a></div>';
		...
		foreach($pictures as $picture_index => $file)
		{
			...
			if($this->params->get('params.count_views'))
			{
				$url_to_original[$picture_index] = sprintf('%s/index.php?option=com_cobalt&task=files.show&id=%d&user_id='.$this->record->user_id.'&fldr=gallery'.$f_key.'&file_key=%s&tmpl=component', JURI::root(TRUE), $file['id'], $key);
			}
		}
...
// Получаем тхумбсы размер 50
			$url =  CImgHelper::getThumb($picture, 0, 60, 'gallery' . $f_key, $this->record->user_id, $options);
			/****************/
			$options = array(
			'strache'    => $this->params->get('params.full_stretch', 1),
			'background' => $this->params->get('params.thumbs_background_color', "#000000"),
	...
	public function onGetImageInfo($post)
	{
		...
		$out['description'] = $this->_getDescription(); // Выводим показ описания
	...
	function onSaveData($post)
	{
		...
		switch($post['type'])
		{
			...
			case 'comment':
				...
				$delete_link = "<div class='del-comment'><a href='javascript:void(0);' class=\"delete-comment1\" onclick='gb" . $this->id . '_' . $this->record->id . ".deleteComment(" . $k . ");'></a>+</div>";
				$delete      = FALSE;
				...
				if($cuser->id == 532 || $cuser->id == 533 || $cuser->id == 544)
				{
					$delete = TRUE;
				}
	...
	public function onDeleteComment($post)
	{
		...
		if($user->id == 532 || $user->id == 533 || $user->id == 544)
		{
			return 1;
		}
	...
	{
		$this->file = JTable::getInstance('Files', 'CobaltTable');
		$this->file->load($id);
	}
...
// todo отключил ред заголовка из галереи
		/*if($this->file->user_id == $this->user->get('id') && $this->params->get('params.allow_edit_title', 0))
		{
			$titlebox = sprintf('<div id="titlebox"><span id="titletextspan">%s</span> <a href="javascript:void(0);" onclick="gb' . $this->id . '_' . $this->record->id . '.editTitle()"><img class="savetitle" src="/images/icons/edit-white.svg";/></a></div>', $titlebox,
			...
			$titlebox .= '<div id="titleedit" class="disactiveInput">
        	    ...
        	    <a href="javascript:void(0);" onclick="gb' . $this->id . '_' . $this->record->id . '.saveTitle();">' . JHtml::image(JURI::root() . 'images/icons/save.svg', JText::_('G_SAVE'), array('align' => 'absmiddle')) . '</a></div>';
		}*/
		$avatar = '';
	...
	private function _getDescription()
	{
		...
		$gurojaya = $this->file->gurojaya;
		$vcolor = $this->file->vcolor;
		$vsugar = $this->file->vsugar;
		$vsort = $this->file->vsort;
		$vvinogrd = $this->file->vvinogrd;
		$vproizvod = $this->file->vproizvod;
		$vorglept = $this->file->vorglept;
		$vgastronom = $this->file->vgastronom;
		$vtemp = $this->file->vtemp;
		$vkisl = $this->file->vkisl;
		$gradus = $this->file->gradus;
		$description = $this->file->description;

// todo отключил ред описания из галереи
		...
		if(isset($gurojaya)    && !empty($gurojaya))    { $gurojaya = '<div class="descrbox"><h5 class="descr_opt">Год урожая</h5><div class="proddescr">'. $gurojaya .'</div></div>';}
		if(isset($vcolor)      && !empty($vcolor))      { $vcolor = '<div class="descrbox"><h5 class="descr_opt">Цвет</h5><div class="proddescr">'. $vcolor .'</div></div>';}
		if(isset($vsugar)      && !empty($vsugar))      { $vsugar = '<div class="descrbox"><h5 class="descr_opt">Сахар</h5><div class="proddescr">'. $vsugar .'</div></div>';}
		if(isset($vsort)       && !empty($vsort))       { $vsort = '<div class="descrbox"><h5 class="descr_opt">Сорт винограда</h5><div class="proddescr">'. $vsort .'</div></div>';}
		if(isset($vvinogrd)    && !empty($vvinogrd))    { $vvinogrd = '<div class="descrbox"><h5 class="descr_opt">Виноградник</h5><div class="proddescr">'. $vvinogrd .'</div></div>';}
		if(isset($vproizvod)   && !empty($vproizvod))   { $vproizvod = '<div class="descrbox"><h5 class="descr_opt">Способ производства</h5><div class="proddescr">'. $vproizvod .'</div></div>';}
		if(isset($vorglept)    && !empty($vorglept))    { $vorglept = '<div class="descrbox"><h5 class="descr_opt">Органолептика</h5><div class="proddescr">'. $vorglept .'</div></div>';}
		if(isset($vgastronom)  && !empty($vgastronom))  { $vgastronom = '<div class="descrbox"><h5 class="descr_opt">Гастрономия</h5><div class="proddescr">'. $vgastronom .'</div></div>';}
		if(isset($vtemp)       && !empty($vtemp))       { $vtemp = '<div class="descrbox"><h5 class="descr_opt">Температура подачи °C</h5><div class="proddescr">'. $vtemp .'</div></div>';}
		if(isset($vkisl)       && !empty($vkisl))       { $vkisl = '<div class="descrbox"><h5 class="descr_opt">Кислотность</h5><div class="proddescr">'. $vkisl .'</div></div>';}
		if(isset($gradus)      && !empty($gradus))      { $gradus = '<div class="descrbox"><h5 class="descr_opt">Объемная доля спирта % об.</h5><div class="proddescr">'. $gradus .'</div></div>';}
		if(isset($description) && !empty($description)) { $description = '<div class="descrbox"><h5 class="descr_opt">Дополнительно</h5><div class="proddescr">'. $description .'</div></div>';}
		
		return '<div id="descrbox">'. $gurojaya.$vcolor.$vsugar.$vsort.$vvinogrd.$vproizvod.$vorglept.$vgastronom.$vtemp.$vkisl.$gradus.$description .'</div>';
	}
	...
	private function _getComments()
	{
		...
		if(!empty($comments))
		{
			...
			foreach($comments as $key => $comment)
			{
				...
				$delete_link = "<div class='del-comment'><a href='javascript:void(0);' class=\"delete-comment1\" onclick='gb" . $this->id . '_' . $this->record->id . ".deleteComment({$key});'>+</a></div>";
				$delete      = FALSE;
	...
	private function _getCommentsForm()
	{
		if(in_array($this->params->get('params.allow_comments'), $this->user->getAuthorisedViewLevels()))
		{
			return '<textarea id="savecommenttext" class="inputbox"></textarea>
				<a class="button-full" href="javascript:void(0);" onclick="if($(\'savecommenttext\').value == \'\'){alert(\'' . JText::_('CCOMMENT_EMPTY') . '\');return false;}gb' . $this->id . '_' . $this->record->id . '.saveComment();">
        	    ' . JText::_('G_SAVE') . '</a>';
		}
		else
		{
	...
	public function _init()
	{
		...
		if($this->request->getCmd('tmpl') != 'component')
		{
			if($this->params->get('params.show_mode', 'gallerybox') == 'gallerybox')
			{
				...
				JFactory::getDocument()->addScript(JURI::root(TRUE) . '/components/com_cobalt/fields/gallery/gallerybox/gallerybox.js');
				RatingHelp::loadFile();
				JFactory::getDocument()->addStyleSheet(JURI::root(TRUE) . '/components/com_cobalt/fields/gallery/gallerybox/gallerybox.css');
				JFactory::getDocument()->addStyleSheet(JURI::root(TRUE) . '/components/com_cobalt/fields/gallery/gallerybox/gallerybox-' . $this->params->get('params.theme', 'Dark') . '.css');
		...
		$this->path = JPATH_ROOT . DIRECTORY_SEPARATOR . JComponentHelper::getParams('com_cobalt')->get('general_upload') . DIRECTORY_SEPARATOR . 'thumbs_cache' . DIRECTORY_SEPARATOR;
