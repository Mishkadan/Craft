...
class JFormFieldCSocusers extends CFormField
{
	...
	public function getModers() {
		...
		{
			$sql = "SELECT  cuser_id, moder_record FROM #__cobalt_moderators WHERE moder_record = $rid";
		} else {
			$sql = "SELECT  cuser_id, moder_record FROM #__cobalt_moderators";
		}
		$db->setQuery($sql);
		return $db->loadObjectList(cuser_id);

	}

	public function getInput()
	{
		JFactory::getDocument()->addScript(JUri::root(true) . '/components/com_cobalt/fields/image/assets/input.js');
		$params = $this->params;
...
//		{
	...
	public function onImport($value, $params, $record = null)
	{
		...
		$upload_path = DIRECTORY_SEPARATOR . JComponentHelper::getParams('com_cobalt')->get('general_upload') . DIRECTORY_SEPARATOR . $this->params->get('params.subfolder');
