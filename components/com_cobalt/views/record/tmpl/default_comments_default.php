...
{
	$menu[0] = '<a href="#commentmodal" data-toggle="modal" role="button" onclick="Cobalt.editComment('.$this->comment->id.')">' . JText::_('CEDIT') . '</a>';
}
...
{
	$replay = '<a class="btn btn-mini btn-primary" href="#commentmodal" data-toggle="modal" role="button" onclick="Cobalt.editComment(0, '.$this->comment->id.', '.$this->item->id.')"><i class="icon-comments-2"></i> ' . JText::_('CREPLY') . '</a>';
}
...
<div style="margin-left: <?php echo $params->get('tmpl_params.comments_indent') * ($this->comment->level - 1)?>px;" id="comment<?php echo $this->comment->id?>-container" class="commentblock">
	<div class="row-fluid comentrows">
		...
		<div class="span<?php echo $this->comment->level > 1 ? 11 : 10; ?> has-context<?php echo  $this->comment->private ? ' private' : null; echo  !$this->comment->published ? ' published' : null?>">
			...
			<?php endif;?>
				...
				<?php if($params->get('tmpl_core.comments_username')):?>
					<div class="commname"><?php echo JText::sprintf('CWRITTENBY', ($this->comment->name ? $this->comment->name : CCommunityHelper::getName($this->comment->user_id, $this->section))); ?></div>
				<?php endif;?>
				
				<?php if($params->get('tmpl_core.comments_date')):?>
					<small class="commdate"><?php echo JText::sprintf('CONDATE', JHtml::_('date', $this->comment->created, $params->get('tmpl_core.comments_time_format'))); ?></small>
				<?php endif;?>
				</div>
