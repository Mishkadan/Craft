...
if(CobaltCommentHelper::laded($this->item->id)) {
	return;
}
?>
...
<?php if($this->type->params->get('comments.comments') == 'core'): ?>
	...
	{
		jQuery.ajax({
			url:'<?php echo JRoute::_("index.php?option=com_cobalt&task=rate.comment&tmpl=component", FALSE); ?>',
...
<div class="content_block1 hide1">
...
<!-- <button id="content_toggle1">Показать все</button> -->
...
$('#content_toggle1').click(function(){
		$('.content_block1').toggleClass('hide1');	
		if ($('.content_block1').hasClass('hide1')) {
			$('#content_toggle1').html('Показать все');
		} else {
			$('#content_toggle1').html('Свернуть отзывы');
		}		
		return false;
	});				
