...
$pos1 = @$this->fields_keys_by_id[$params->get('tmpl_params.field_id_position_1')];
...
<table class="table-records <?php ECHO $params->get('tmpl_params.table_class') ?> section-<?php echo $this->section->id; ?> <?php echo implode(' ', (array)$params->get('tmpl_core.table_style')); ?>  cob-section-<?php echo $this->section->id; ?>">
	<?php if($params->get('tmpl_params.table_header', 1)):?>
		<thead>
			<div class="ww1">
 				<?php if($pos1 && $fh->isExists($pos1)): ?>
					<th nowrap="nowrap" class="cob-p1">
				...
				<th class="cob-title-th">
					<?php if($params->get('tmpl_params.item_icon_title')): ?>
						<img src="<?php echo JURI::root();?>media/mint/icons/16/<?php echo $params->get('tmpl_params.item_icon_title_icon') ?>" align="absmiddle" />
					<?php endif;?>
				...
				<?php if($params->get('tmpl_core.item_author') == 1):?>
					<th nowrap="nowrap" class="cob-author-th">
				...
				<?php foreach ($total_fields_keys AS $field):?>
					...
					<th nowrap="nowrap" class="cob-f<?php echo $field->id; ?>">
				...
				<?php if($params->get('tmpl_core.item_extime') == 1):?>
					<th nowrap="nowrap" class="cob-extime-th">
				...
				<?php if($params->get('tmpl_core.item_favorite_num') == 1):?>
					<th nowrap="nowrap" class="cob-book-th">
				...
				<?php if($params->get('tmpl_core.item_hits') == 1):?>
					<th nowrap="nowrap" class="cob-hit-th">
	...
	<tbody>
		<?php foreach ($this->items AS $item):?>
			...
			?>
          <?//= $item->url;?>
			<div class="cat-list-card" onclick="javascript:location.href ='<? echo JRoute::_($item->url);?>'"  >
				<?php if($pos1 && $p1):?>
					<?php if(isset($item->fields_by_key[$pos1])):?>
						<?php $result = $item->fields_by_key[$pos1]->result;?>
						<div class="catleft"><?php echo $result; ?></div>
                ...
                <div class="has-context cob-title-td">
					<div class="relative_ctrls">
						...
						<!--<?php //if($this->user->get('id')):?> скрытие меню над карточкой товара
...
<!--									<a href="--><?php //echo JRoute::_($item->url);?><!--"><button class="chocolate">Подробнее</button></a>-->
								<?php endforeach;?>
							</dl>
						<?php endif;?>
				...
				<?php if($params->get('tmpl_core.item_rating') == 1):?>
					<div nowrap="nowrap" class="cob-rating-td"><?php echo $item->rating;?></div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_author_avatar') == 1):?>
					<div class="cob-avatar-td">
						<img src="<?php echo CCommunityHelper::getAvatar($item->user_id, $params->get('tmpl_core.item_author_avatar_width', 40), $params->get('tmpl_core.item_author_avatar_height', 40));?>" />
					</div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_author') == 1):?>
					<div nowrap="nowrap"  class="cob-avatar-td"><?php echo CCommunityHelper::getName($item->user_id, $this->section);?>
				...
				<?php if($params->get('tmpl_core.item_type') == 1):?>
					<div nowrap="nowrap" class="cob-type-td"><?php echo $item->type_name;?>
				...
				<?php foreach ($total_fields_keys AS $field):?>
					<?php if(in_array($field->key, $exclude)) continue; ?>
					<div class="<?php echo $field->params->get('core.field_class')?>" <?php if($params->get('tmpl_core.digit_right') && $field->type == 'digits') echo ' align="right"'?>><?php if(isset($item->fields_by_key[$field->key]->result)) echo $item->fields_by_key[$field->key]->result ;?></div>
				<?php endforeach;?>

				<?php if($params->get('tmpl_core.item_user_categories') == 1 && $this->section->params->get('personalize.pcat_submit')):?>
					<div class="cob-ucat-td"><?php echo $item->ucatname;?></div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_categories') == 1 && $this->section->categories):?>
					<div class="cob-cat-th"><?php echo implode(', ', $item->categories_links);?></div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_ctime') == 1):?>
					<div class="cob-ctime-td"><?php echo JHtml::_('date', $item->created, $params->get('tmpl_core.item_time_format'));?></div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_mtime') == 1):?>
					<div class="cob-mtime-td"><?php echo JHtml::_('date', $item->modify, $params->get('tmpl_core.item_time_format'));?></div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_extime') == 1):?>
					<div class="cob-extime-td"><?php echo ( $item->expire ? JHtml::_('date', $item->expire, $params->get('tmpl_core.item_time_format')) : JText::_('CNEVER'));?></div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_comments_num') == 1):?>
					<div class="cob-comm-td"><?php echo CommentHelper::numComments($this->submission_types[$item->type_id], $item);?></div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_vote_num') == 1):?>
					<div class="cob-vote-td"><?php echo $item->votes;?></div>
				<?php endif;?>


				<?php if($params->get('tmpl_core.item_favorite_num') == 1):?>
					<div class="cob-book-td"><?php echo $item->favorite_num;?></div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_follow_num') == 1):?>
					<div class="cob-follow-td"><?php echo $item->subscriptions_num;?></div>
				<?php endif;?>

				<?php if($params->get('tmpl_core.item_hits') == 1):?>
					<div class="cob-hit-td"><?php echo $item->hits;?></div>
				<?php endif;?>
			</div>
		<?php endforeach;?>
	</tbody>
