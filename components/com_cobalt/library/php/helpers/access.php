...
class MECAccess
{
	...
	public static function allowNew($type, $section)
	{
		$user = JFactory::getUser();
		$db        = JFactory::getDBO();
		//$sql      = "SELECT  cuser_id, moder_record FROM #__cobalt_moderators WHERE moder_record = $rid"; // TODO сделаьт еще отбор по record_id
		$sql      = "SELECT  cuser_id, moder_record FROM #__cobalt_moderators";
		$db->setQuery($sql);
		$moderators = $db->loadObjectList();
		$moder_list =array();
		foreach ($moderators as $vl)
		{
			$moder_list[] = $vl->cuser_id;
		}
	...
	public static function allowEdit($record, $type, $section)
	{
		...
		//$moders  = JFormFieldCSocusers::getModers();
		$rid = $record->id;
		$db        = JFactory::getDBO();
		...
		if($app->input->get('task') == 'edit') {
			if($record->id && ($type->params->get('submission.public_edit') &&
