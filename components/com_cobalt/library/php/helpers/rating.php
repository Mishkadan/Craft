...
class RatingHelp
{
	...
	public static function loadMultiratings($record, $type, $section, $render = false)
	{
		...
		else
		{
			...
			{
				$out .= '<div class="rating10"><div id="rating-text-' . $record->id . '" class="ratingball">' . JText::sprintf('CRAINGDATA', $record->votes_result, $record->votes) . '</div><div id="tchknol" class="ratingball">' . JText::sprintf('.0') . '</div></div>';
			}
