...
class HTMLFormatHelper
{
	...
	public static function followsection(&$section)
	{
		...
		$format = '<a id="followsec-%d" type="button" class="btn btn-small%s" onclick="Cobalt.followSection(%d);" onmouseover="%s" onmouseout="%s">
		<img id="follow_%d" align="absmiddle" src="%s/media/mint/icons/16/follow%d.png"/></a>';
	...
	public static function followcat($cat_id, $section)
	{
		...
		$format = '<a id="followcat-%d" type="button" class="btn btn-small%s" onclick="Cobalt.followCat(%d, %d);" onmouseover="%s" onmouseout="%s">
	...
	public static function followuser($user_id, $section)
	{
		...
		$format = '<a id="followuser-%d" type="button" class="btn btn-small%s" onclick="Cobalt.followUser(%d, %s);" onmouseover="%s" onmouseout="%s">
	...
	public static function follow($record, $section)
	{
        ...
        $text_current = $record->subscribed ? JText::_('CMSG_CLICKTOUNFOLLOW') : JText::_('CMSG_CLICKTOFOLLOW');
		$svg  = '
            <svg class="icon icon__bell' . ((int)($record->subscribed > 0) ? ' icon__bell--fill' : '') . '" aria-hidden="true">
                <use href="#ui__bell"></use>
                <use href="#ui__bell-fill"></use>
                <use href="#ui__loading"></use>
            </svg>
			<span class="tmp__button__title" data-title-subscribe="' . JText::_('CUSERFOLLOWING_TITLE', 'Follow') . '" data-title-unsubscribe="' . JText::_('CUSERUNFOLLOWING_TITLE', 'Unfollow') . '" data-status-subscribe="' . ($record->subscribed ? 'true' : 'false' ) . '">' . ($record->subscribed ? JText::_('CUSERUNFOLLOWING_TITLE', 'Unfollow') : JText::_('CUSERFOLLOWING_TITLE', 'Follow')) . '</span>
            ';

		return sprintf('
            <button id="follow_record_%1$d" class="tmp__button tmp__button--follow" onclick="Cobalt.followRecord(%1$d, %2$d);" aria-label="%3$s" data-original-title="%3$s" title="%3$s" rel="tooltip">%4$s</button>
            ',
            // %1$d
			$record->id,
            // %2$d
            JFactory::getApplication()->input->getInt('section_id'),
            // %3$s
            $text_current,
            // %4$s
            $svg,
	...
	public static function repost($record, $section)
	{
		...
		return sprintf('<a class="btn btn-co-control btn-mini" id="repost_%d" type="button" onclick="Cobalt.RepostRecord(%d, %d);">%s</a>',
	...
	public static function bookmark($record, $type, $params)
	{
		...
		return sprintf('<a class="btn btn-co-control btn-mini" type="button" onclick="Cobalt.bookmarkRecord(%d, \'%s\', %d);">%s</a>',
