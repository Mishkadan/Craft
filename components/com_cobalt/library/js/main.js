...
(function($) {
    ...
    Cobalt.followRecord = (id, section) => {
		const followButton = document.getElementById('follow_record_' + id);
		if (followButton)
		{
			const tooltipInner = document.querySelector('.tooltip-inner');
			const followIcon = followButton?.querySelector('.icon');
			const followTitle = followButton?.querySelector('.tmp__button__title');

			followIcon.classList.add('icon__loading');
			...
			if (followIcon)
			{
				followIcon.classList.remove('icon__loading');
				followIcon.classList.toggle('icon__bell--fill');
			}

			if (followTitle)
			{
				followTitle.textContent = (followTitle.dataset.statusSubscribe === 'true') ? followTitle.dataset.titleSubscribe : followTitle.dataset.titleUnsubscribe;

				followTitle.dataset.statusSubscribe = (followTitle.dataset.statusSubscribe === 'true') ? 'false' : 'true';
			}

			followButton.setAttribute('title', json.title);
			followButton.setAttribute('aria-label', json.title);
			followButton.setAttribute('data-original-title', json.title);

			tooltipInner.textContent = json.title;
