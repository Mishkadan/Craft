...
<div class="joms-stream__actions">
    ...
    ?>
    <i class="lenta far fa-comment" onclick="showCommentInput('<?= $shareId?>')"></i>
    <!-- share -->
    <?php
    ...
    if ($my->id > 0 && $my->authorise('community.postcommentcreate', 'com_community') && $my->id != $act->actor &&
        ( ($act->access == 0 || $act->access == 10 || $act->access == 20 ) && ($act->group_access == 0 && $act->event_access == 0))
...
<script>
    ...
    function showCommentInput(id) {
       var comment =  document.querySelector('.joms-comment__reply.joms-js--newcomment.joms-js--newcomment-'+id);
