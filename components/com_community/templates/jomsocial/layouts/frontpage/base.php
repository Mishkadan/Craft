                        ...
                        <ul class="unstyled joms-activity-filter-dropdown joms-postbox-dropdown" style="display: none">
                            ...
                            */?>
                            <li <?php echo ($filterKey == 'keyword' || $filterKey == 'hashtag') ? 'class="active"' : '' ?>
                                ...
                                <div class="joms-select--wrapper" style="margin-bottom:4px; display: none">
                                    <select class="joms-select">
                                        <option selected value="keyword" <?php echo ( $filterKey == 'keyword' ) ? 'selected="selected"' : '' ?>><?php echo JText::_("COM_COMMUNITY_FILTERBY_KEYWORD"); ?></option>
                                        <option value="hashtag" <?php echo ( $filterKey == 'hashtag' ) ? 'selected="selected"' : '' ?>><?php echo JText::_("COM_COMMUNITY_FILTERBY_HASHTAG"); ?></option>
                                    </select>
                                </div>
                                <input type="text" class="joms-input" style="text-align:center !important; box-shadow: none !important;" value="<?php echo ($filterKey == 'keyword' || $filterKey == 'hashtag') ? $filterValue : '' ?>"
...
<!-- dummy comment -->
<div class="vue-mount-postmenu" style="display:none;"></div>
...
<script>
 ...
 jQuery(function ($){
     ...
     /* wait loading postbox then "pointer-events->auto" in /components/com_community/assets/postbox/js/views/postbox/postmenu.js */
     $('#addpost').css('pointer-events','none');

    // remove success login alert window
     setTimeout(function (){
         let alertblock =  $('#system-message').find('.alert.alert-message');
         if(  <?= $alreadyLogin ?> == 1) {
              alertblock.fadeOut( function() {
                  alertblock.find('a.close').click();
              });
         }
     }, 1000);
})
