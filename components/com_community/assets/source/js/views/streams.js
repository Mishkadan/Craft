...
function uninitialize() {
    ...
    function initPhotoArranger() {
        var initialized = '.joms-js--initialized',
            ...
            res = $('.lenta_attachments');
        const count = 2;

        res.each( function () {
            let el = $(this).find('li').not('.def');
            for (let s = 0, e = count; s < el.length; s += count, e += count) {
                let res_el = el.slice(s,e);
                res_el.length === 2 ? res_el.css('width', '50%').find('img').css({
                    height       : res_el.width(),
                    width        : '100%',
                    'object-fit' : 'cover'
                }) : '';
