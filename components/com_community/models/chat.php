...
class CommunityModelChat extends JCCModel implements CNotificationsInterface
{
    ...
    public function getLastChat($chatId, $offset = 0,  $limit = 20, $seen = 1, $is_seen = 1 )
    {
        if ($seen) {
            $this->seen($chatId);
        }
        ...
        $query = "SELECT a.`id`, a.`chat_id`, a.`user_id`, a.`action`, a.`content`, a.`params`, a.`created_at`,  a.`is_seen` 
    ...
    public function getChat($id)
    {
        ...
        $query = "SELECT a.`name`, a.`type`, a.`owner`, a.`thumb`, b.`chat_id`, b.`enabled`, b.`mute`, a.`last_msg`
        ...
        if ($chat->type === 'single') {
            ...
            $chat->thumb = $chat->thumb ? JUri::root(). 'images/chatavatars/'.$id.'/'.$chat->thumb : JUri::root() . 'components/com_community/assets/group_thumb.jpg';
        }
    ...
    public function getMyChatList($ids = array(), $limit = false, $mode = false, $userId = null)
    {
        ...
        $query = "SELECT c.id as chat_id, c.type, c.name, c.last_msg, c.thumb, cp.enabled, cp.mute
        ...
        foreach ($list as $item) {
            ...
            if ($item->type === 'single') {
                ...
                $item->thumb =  $item->thumb ? JUri::root(). 'images/chatavatars/'.$item->chat_id.'/'.$item->thumb : JUri::root() . 'components/com_community/assets/group_thumb.jpg';
    ...
    public function addActivity($chatid, $user_id, $to, $action, $content = '', $params = '', $created_at = '', $is_seen = '') //  $is_seen = 'null'
    {
        ...
        $data = array(
            ...
            'created_at' => $created_at ? $created_at : date('Y-m-d H:i:s'), // craft 09.07.22 fix time chat ('$created_at' fixed in controller chat)
            'is_seen' => $is_seen,
        );
    ...
    {
        $query = 'SELECT LAST_INSERT_ID() FROM `#__community_chat_activity`';  //  Craft
        $db = JFactory::getDbo();
        $result = $db->setQuery($query)->loadResult();
        return (int)$result;
    }
    public function GetUnreadMsg($chatid, $user_id) //  Craft
    {
    ...
    public function SetMessagesSeen($chatid, $user_id) //  Craft
    {
        $ids = $this->GetUnreadMsg($chatid, $user_id);
        $db = JFactory::getDbo();
        foreach ($ids as $id) {
...
//		$query = 'UPDATE `#__community_chat_activity` SET is_seen = 2 WHERE id = '.$lastid;//.' AND chat_id = ' . $chatid.' AND user_id = '.$user_id.' AND action = sent';//.' AND action = '.$action;
//		$db = JFactory::getDbo();
//		$db->setQuery($query)->execute();

        $query = 'UPDATE `#__community_chat_activity` SET is_seen = 2 WHERE  chat_id = ' . $chatid.' AND user_id = '.$user_id;
        $db = JFactory::getDbo();
        $db->setQuery($query)->execute();
    ...
    public function addChat($chatid, $message, $attachment, $is_seen = 1) // $is_seen
    {
        ...
        $my = CFactory::getUser();
        $tousers = json_encode(self::getChatPartner($chatid));
        $params = new CParameter();
        $params->set('attachment', $attachment);
        $params = $params->toString();

        $activity = $this->addActivity($chatid, $my->id, $tousers, 'sent', $message, $params, date('Y-m-d H:i:s'), $is_seen); //  $is_seen Craft + fix datetime save //09.07.22
        ...
        $data->user_id =  $activity->user_id; // craft add myid for push
        $data->message =  $activity->content; // craft add chat text for push
    ...
    public function createChat($message, $attachment, $partner, $name, $is_seen)
    {
        if ($this->isReachLimit()) {
            $error = new stdClass;
            $error->error = JText::_('COM_COMMUNITY_PM_LIMIT_REACHED');
        ...
        $chat->chat_id = $chatid;
        $result = $this->addChat($chatid, $message, $attachment, $is_seen);
        $result->chat = $chat;
    ...
    public function seen($chat_id)
    {
        ...
        if ($last_user_activity < $last_chat) {
            ...
            $this->addActivity($chat_id, $my->id, $my->id,'seen');

            $this->SetMessagesSeen($chat_id, $my->id);
    ...
    public function addPeople($chat_id, $ids)
    {
        ...
        if (count($exist_user)) {
            foreach ($exist_user as $uid) {
                ...
                $this->addActivity($chat_id, $uid, $uid, 'add');
            }
        }
        ...
        if (count($id_diff)) {
            ...
            foreach ($id_diff as $uid) {
                ...
                $data->chat_id = $chat_id;
                $data->user_id = $uid;
                $data->params = json_encode(array('owner' => CFactory::getUser()->id)); // Craft add owner
    ...
    // craft remake to search all users
    public function getCraftusersId($keyword)
    {
        ...
        if(!empty($keyword)){
            $andName    = ' WHERE' . $db->quoteName( $nameField ) . ' LIKE ' . $db->Quote( '%'.$keyword.'%' ) ;
        }
    ...
    public function changeGroupChatName($chat_id, $name) {
        ...
        if ( $name && $this->isGroupChat($chat_id) && $this->isGroupChatMember($my->id, $chat_id)) {
            ...
            try {
                ...
                $this->addActivity($chat_id, $my->id,  $my->id, 'change_chat_name', '', $params->toString());
    ...
    public function showMembersGroupChat($chatid) {

        $db = JFactory::getDbo();
        $chatpartners = implode(',', $this->getChatPartner($chatid));

        $query = "SELECT a.`name`, b.`userid`, b.`thumb`, b.`alias`
            FROM `#__users` a
            INNER JOIN `#__community_users` b ON a.`id` = b.`userid`
            WHERE  a.`id` IN (".$chatpartners.")
             ORDER BY a.`name` ASC";
        $db->setQuery($query);

        $result = new stdClass();
        $result->users = $db->loadObjectList();
        $result->owner =  CFactory::getUser()->id === $this->getChat($chatid)->owner ||  JFactory::getUser()->get('isRoot');
    ...
    public function deleteUserFromChat($userid, $chatid) {
        ...
        if ($my->id === $this->getChat($chatid)->owner) {
        $db->setQuery( 'DELETE FROM `#__community_chat_participants` WHERE  `chat_id` = '.$chatid.' AND `user_id` ='.$userid);
        $db->execute();

            return $db->execute() ? json_encode(array('ok' => 1, 'chat'=> $chatid, 'user' => $userid)) : json_encode(array('ok' => 0, 'error' => 'Ошибка! Не удалось удалить пользователя'));
        } else return json_encode(array('ok' => 0, 'error' => 'Вы не являетесь создателем чата и не можете его удалить'));
    }

    public function saveGroupChatAva($file, $chatid) {

        $db = JFactory::getDbo();
        $my = CFactory::getUser()->id;

        if (!$chatid) {
            $query = 'UPDATE #__community_users SET `chat_cover` = ' . $db->quote($file) . ' WHERE `userid` = ' . $my;
        }
        else $query = 'UPDATE #__community_chat SET `thumb` = ' . $db->quote($file) . ' WHERE `id` = ' . $chatid;
