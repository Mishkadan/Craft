    ...
    implements CLimitsInterface, CNotificationsInterface
{
    ...
    public function getInviteListByName($namePrefix ,$userid, $cid, $limitstart = 0, $limit = 8){
        ...
        $nameField = $config->getString('displayname');
        if(!empty($namePrefix)){
            //$andName	= ' AND ' . $db->quoteName( $nameField ) . ' LIKE ' . $db->Quote( '%'.$namePrefix.'%' ) ; // craft to search all users 09.07.22
            $andName	= ' WHERE ' . $db->quoteName( $nameField ) . ' LIKE ' . $db->Quote( '%'.$namePrefix.'%' ) ;
        }
//        $query	=   'SELECT DISTINCT(a.'.$db->quoteName('connect_to').') AS id  FROM ' . $db->quoteName('#__community_connection') . ' AS a ' // craft to search all users 09.07.22
//            . ' INNER JOIN ' . $db->quoteName( '#__users' ) . ' AS b '
...
//        ;
        ...
        $query	=   'SELECT COUNT(DISTINCT(a.'.$db->quoteName('connect_to').'))  FROM ' . $db->quoteName('#__community_connection') . ' AS a '
            ...
            . ' AND b.'.$db->quoteName('block').'=' .$db->Quote('0')
            . $andName
            . ' AND NOT EXISTS ( SELECT d.'.$db->quoteName('blocked_userid') . ' as id'
            //. ' WHERE NOT EXISTS ( SELECT d.'.$db->quoteName('blocked_userid') . ' as id' // craft to search all users
            . ' FROM '.$db->quoteName('#__community_blocklist') . ' AS d  '
            . ' WHERE d.'.$db->quoteName('userid').' = '.$db->Quote($userid)
            . ' AND d.'.$db->quoteName('blocked_userid').' = a.'.$db->quoteName('connect_to').')'
           // . $andName // craft to search all users
            ;
