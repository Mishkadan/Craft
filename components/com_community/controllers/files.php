...
class CommunityFilesController extends CommunityBaseController {
    ...
    public function multiUpload() {
        ...
        // log  files
        $log = '********************************************* 1'.PHP_EOL;
        $log .= date('Y-m-d H:i:s') .' user '.JFactory::getUser()->id.'('.$file->name.') upload to '.$file->filepath.PHP_EOL;
        $log .= str_replace(array('	', PHP_EOL), '', print_r($_file, true));
        $log .= str_replace(array('	', PHP_EOL), '', print_r($file, true));
        file_put_contents(__DIR__ . '/file.log', $log . PHP_EOL, FILE_APPEND);
...
// Craft commett "if" to upload all file types
//        if ($type != 'activities') {
...
//        }
        ...
        if($file->type == 'multimedia') {

            include_once (JPATH_ROOT.'/components/com_community/libraries/videos.php');

            $videos = new CVideos ();
            $videopath = JPATH_ROOT . '/'.$file->filepath;
            $videoFolder = $config->get('videofolder');

            // Create thumbnail
            $thumbSize = CVideos::thumbSize();
            $thumbPath = JPATH::clean(JPATH_ROOT.'/'.$videoFolder.'/'.VIDEO_FOLDER_NAME.'/'. $my->id . '/thumbs/');
            $thumbresult    = $videos->createVideoThumb($videopath, $thumbPath, 3, $thumbSize);

            // convert and compress video
            $videoSize = CVideosHelper::getVideoSize();
            $deleteOriginal	= $config->get('deleteoriginalvideos');
            $videoOutFolder	= JPATH::clean(JPATH_ROOT.'/'.$videoFolder.'/'.VIDEO_FOLDER_NAME.'/'. $my->id . '/converted/');
            //$videoFilename	= $videos->convertVideo($videopath , $videoOutFolder, $videoSize, $deleteOriginal);

            $file->thumb 	= JPATH::clean($videoFolder.'/'.VIDEO_FOLDER_NAME.'/'. $my->id . '/thumbs/'. $thumbresult);
            $file->filepath	= $videoFilename ? JPATH::clean($videoFolder.'/'.VIDEO_FOLDER_NAME.'/'. $my->id . '/converted/'.$videoFilename) : $file->filepath;
        }
        ...
        $json = array('id' => $table->id, 'name' => $table->name, 'path' => JUri::root().$table->filepath,);

        if($file->type == 'multimedia') $json['thumb'] = $file->thumb ? JUri::root().$table->thumb : 'Ошибка создания превью FFMPEG';
